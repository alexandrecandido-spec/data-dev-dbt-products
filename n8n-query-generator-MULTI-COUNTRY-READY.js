...
// CORREGIDO: Solo Shares Analysis usa lógica antigua para Performance
// 🌎 MULTI-COUNTRY: AR, BR, MX - READY
// 📚 TABLAS ESPECÍFICAS POR PAÍS - EVENTOS TABLA DINÁMICA
// 🗓️ EVENT DAYS FILTER: Permite filtrar por días específicos del evento

// =======================
// ⚙️ CONFIG - MULTI COUNTRY
// =======================
const CFG = {
  // 🌎 PAÍS SELECTOR - CAMBIAR SEGÚN NECESIDAD
  COUNTRY: "AR", // "AR" | "BR" | "MX"
  
  // 🗓️ DÍAS DE EVENTO SELECTOR - CAMBIAR SEGÚN NECESIDAD
  EVENT_DAYS: "ALL", // "ALL" | [0,1,2] | [3,4,5] | [4] | etc.
  
  // 🎯 EVENTOS POR PAÍS
  EVENTS: {
    AR: {
      CURRENT: "cybermonday-2025",
      LAST: "cybermonday-2024"
    },
    BR: {
      CURRENT: "blackfriday-2024", 
      LAST: "blackfriday-2023"
    },
    MX: {
      CURRENT: "buenfin-2024",
      LAST: "buenfin-2023"
    }
  },
  
  // 🌎 Tablas por país - TODAS ESPECÍFICAS POR PAÍS
  TABLES: {
    // 🔄 TABLAS ACTUAL - ESPECÍFICAS POR PAÍS
    ACTUAL: {
      AR: {
        CORE: "data_products_dev.testing_marketing.special_events_actual",
        SESSIONS: "data_products_dev.testing_marketing.special_events_sessions_actual",
        PRODUCTS: "data_products_dev.testing_marketing.special_events_products_actual"
      },
      BR: {
        CORE: "data_products_dev.testing_marketing.special_events_actual_core_br",
        SESSIONS: "data_products_dev.testing_marketing.special_events_sessions_actual_br",
        PRODUCTS: "data_products_dev.testing_marketing.special_events_products_name_actual_br"
      },
      MX: {
        CORE: "data_products_dev.testing_marketing.special_events_actual_core_mx",
        SESSIONS: "data_products_dev.testing_marketing.special_events_sessions_actual_mx",
        PRODUCTS: "data_products_dev.testing_marketing.special_events_products_name_actual_mx"
      }
    },
    
    // 📚 TABLAS HISTÓRICAS - ESPECÍFICAS POR PAÍS
    HISTORICO: {
      AR: {
        CORE: "data_products_dev.testing_marketing.special_events_historico",
        SESSIONS: "data_products_dev.testing_marketing.special_events_sessions_historico",
        PRODUCTS: "data_products_dev.testing_marketing.special_events_products_name_historico"
      },
      BR: {
        CORE: "data_products_dev.testing_marketing.special_events_historico_br",
        SESSIONS: "data_products_dev.testing_marketing.special_events_sessions_historico_br",
        PRODUCTS: "data_products_dev.testing_marketing.special_events_products_name_historico_br"
      },
      MX: {
        CORE: "data_products_dev.testing_marketing.special_events_historico_core_mx",
        SESSIONS: "data_products_dev.testing_marketing.special_events_sessions_historico_mx",
        PRODUCTS: "data_products_dev.testing_marketing.special_events_products_name_historico_mx"
      }
    }
  },
  
  // 🎯 Tabla de eventos por país
  EVENTS_TABLE: {
    AR: "hive_metastore.moltres.mwp_special_date",
    BR: "hive_metastore.moltres.mwp_special_date", 
    MX: "data_products_dev.testing_marketing.special_date_dates"  // 🚨 VERIFICAR SI EXISTE
    // MX: "hive_metastore.moltres.mwp_special_date"  // 👈 FALLBACK temporal
  }
};

// 🎯 Eventos dinámicos según país seleccionado
const CURRENT_EVENT = CFG.EVENTS[CFG.COUNTRY].CURRENT;
const LAST_EVENT = CFG.EVENTS[CFG.COUNTRY].LAST;

// 🗓️ Helper para generar filtro de días
const getEventDaysFilter = () => {
  if (CFG.EVENT_DAYS === "ALL") {
    return ""; // Sin filtro adicional
  }
  
  if (Array.isArray(CFG.EVENT_DAYS)) {
    const daysList = CFG.EVENT_DAYS.join(", ");
    return `AND cd.special_date_day IN (${daysList})`;
  }
...
// =======================
// 🔧 RELATIVE TIME LOGIC HELPER
// =======================

// 🌎 Helper function to generate relative time logic for any data source - MULTI COUNTRY
const getRelativeTimeLogic = (dataSource = "CORE") => {
  // 📋 Tabla ACTUAL (específica por país)
  const actualTable = dataSource === "CORE" ? CFG.TABLES.ACTUAL[CFG.COUNTRY].CORE : 
                     dataSource === "SESSIONS" ? CFG.TABLES.ACTUAL[CFG.COUNTRY].SESSIONS : 
                     CFG.TABLES.ACTUAL[CFG.COUNTRY].PRODUCTS;
  
  // 📚 Tabla HISTÓRICA (específica por país)
  const historicoTable = dataSource === "CORE" ? CFG.TABLES.HISTORICO[CFG.COUNTRY].CORE : 
                        dataSource === "SESSIONS" ? CFG.TABLES.HISTORICO[CFG.COUNTRY].SESSIONS : 
                        CFG.TABLES.HISTORICO[CFG.COUNTRY].PRODUCTS;
  
  return `
events_config AS (
  SELECT '${CURRENT_EVENT}' AS current_event, '${LAST_EVENT}' AS last_event
),
all_data AS (
...
// =======================
// 🔧 OLD LOGIC HELPER (SOLO para Shares Analysis)
// =======================
...
current_event_limit AS (
  ...
  FROM filtered_data CROSS JOIN events_config ec WHERE special_date_name = ec.current_event
)`;
};

// =======================
// 🌎 MULTI-COUNTRY GROUPING HELPERS
// =======================

// 💳 Payment Gateway Grouping - MULTI COUNTRY
const getPaymentGatewayGrouping = () => {
  return `
  CASE
    -- 🇦🇷 ARGENTINA
    WHEN cd.country = 'AR' THEN
    CASE
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) IN ('mercadopago', 'mercado-pago', 'mp') THEN 'Mercado Pago'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) = 'pago-nube' THEN 'Pago Nube'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) = 'pagos-personalizados' THEN 'Efectivo / Transferencia'
      ELSE 'Other'
    END
    
    -- 🇧🇷 BRASIL
    WHEN cd.country = 'BR' THEN
    CASE
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) = 'appmax' THEN 'App Max'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) IN ('pagseguro-prod', 'pagseguro') THEN 'Pago Seguro'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) = 'pagar-me' THEN 'Pagar Me'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) = 'nuvem-pago' THEN 'Nuvem Pago'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) IN ('mercadopago', 'mercado-pago-alpha') THEN 'Mercado Pago'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) IN ('pdv-pagos-personalizados', 'pagos-personalizados', 'not-provided') THEN 'Dinheiro / Transferência'
      ELSE 'Other'
    END
    
    -- 🇲🇽 MÉXICO
    WHEN cd.country = 'MX' THEN
    CASE
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) IN ('mercado-pago-alpha', 'mercadopago') THEN 'Mercado Pago'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) IN ('not-provided', 'pagos-personalizados', 'pdv-pagos-personalizados') THEN 'Efectivo / Transferencia'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) = 'pago-nube' THEN 'Pago Nube'
      WHEN LOWER(COALESCE(cd.gateway_provider_grouping, '')) = 'paypal' THEN 'PayPal'
      ELSE 'Other'
    END
...
// 🚚 Shipping Method Grouping - MULTI COUNTRY
const getShippingMethodGrouping = () => {
  return `
  CASE
    -- 🇦🇷 ARGENTINA
    WHEN cd.country = 'AR' THEN
    CASE
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'custom' THEN 'Envío Personalizado'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'correo-argentino' THEN 'Correo Argentino'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'branch' THEN 'Retiro Sucursal'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'andreani' THEN 'Andreani'
      WHEN COALESCE(cd.shipping_method_grouping, '') IN ('oca','oca-partner-ar') THEN 'Oca'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'envo-nube-argentina' THEN 'Envío Nube'
      ELSE 'Otros'
    END
    ...
    WHEN cd.country = 'BR' THEN
    CASE
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'mandabem' THEN 'Mandabem'
      WHEN COALESCE(cd.shipping_method_grouping, '') IN ('draft', 'undefined', 'custom') THEN 'Envio Personalizado'
      WHEN COALESCE(cd.shipping_method_grouping, '') IN ('branch', 'pickup-point') THEN 'Retiro en Sucursal'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'frenet' THEN 'Frenet'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'mandae' THEN 'Mandae'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'melhor-envio' THEN 'Melhor Envio'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'nuvem-envio' THEN 'Nuvem Envio'
      ELSE 'Other'
    END
    ...
    WHEN cd.country = 'MX' THEN
    CASE
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'envia-com' THEN 'Envia-Com'
      WHEN COALESCE(cd.shipping_method_grouping, '') IN ('draft', 'undefined', 'custom', 'table_default') THEN 'Envio Personalizado'
      WHEN COALESCE(cd.shipping_method_grouping, '') IN ('branch', 'pickup-point') THEN 'Retiro en Sucursal'
      WHEN COALESCE(cd.shipping_method_grouping, '') IN ('skydropx', 'skydropxcolmx') THEN 'SkyDropx'
      WHEN COALESCE(cd.shipping_method_grouping, '') = 'envo-nube' THEN 'Envio Nube'
      ELSE 'Other'
    END
...
// 🏛️ Province Grouping - MULTI COUNTRY
const getProvinceGrouping = () => {
  return `
  CASE
    -- 🇦🇷 ARGENTINA
    WHEN cd.country = 'AR' THEN
    CASE
      ...
      WHEN cd.shipping_province IN ('Capital Federal', 'Gran Buenos Aires', 'CABA','CIUDAD AUTÓNOMA DE BUENOS AIRES','CIUDAD AUTONOMA DE BUENOS AIRES') THEN 'CABA y GBA'
      WHEN cd.shipping_province IN ('Córdoba', 'Cordoba') THEN 'Córdoba'
      WHEN cd.shipping_province IN ('Santa Fé', 'Santa Fe') THEN 'Santa Fé'
      WHEN cd.shipping_province IN ('Buenos Aires', 'Buenos aires') THEN 'Buenos Aires'
      WHEN cd.shipping_province = 'Mendoza' THEN 'Mendoza'
      ELSE 'Other'
    END
    ...
    CASE
      WHEN cd.shipping_province IS NULL THEN 'Other'
      WHEN UPPER(cd.shipping_province) = 'SÃO PAULO' THEN 'São Paulo'
      WHEN UPPER(cd.shipping_province) = 'MINAS GERAIS' THEN 'Minas Gerais'
      WHEN UPPER(cd.shipping_province) = 'RIO DE JANEIRO' THEN 'Rio de Janeiro'
      WHEN UPPER(cd.shipping_province) = 'PARANÁ' THEN 'Paraná'
      WHEN UPPER(cd.shipping_province) = 'RIO GRANDE DO SUL' THEN 'Rio Grande do Sul'
      WHEN UPPER(cd.shipping_province) = 'SANTA CATARINA' THEN 'Santa Catarina'
      WHEN UPPER(cd.shipping_province) = 'BAHIA' THEN 'Bahia'
      WHEN UPPER(cd.shipping_province) = 'CEARÁ' THEN 'Ceará'
      ELSE 'Other'
    END
    ...
    CASE
      WHEN cd.shipping_province IS NULL THEN 'Other'
      WHEN cd.shipping_province = 'Ciudad de México' THEN 'Ciudad de México'
      WHEN cd.shipping_province = 'México' THEN 'México'
      WHEN cd.shipping_province = 'Jalisco' THEN 'Jalisco'
      WHEN cd.shipping_province = 'Nuevo León' THEN 'Nuevo León'
      WHEN cd.shipping_province = 'Veracruz' THEN 'Veracruz'
      WHEN cd.shipping_province = 'Guanajuato' THEN 'Guanajuato'
      WHEN cd.shipping_province = 'Puebla' THEN 'Puebla'
      WHEN cd.shipping_province = 'Sonora' THEN 'Sonora'
      WHEN cd.shipping_province = 'Querétaro' THEN 'Querétaro'
      WHEN cd.shipping_province = 'Baja California' THEN 'Baja California'
      WHEN cd.shipping_province = 'Sinaloa' THEN 'Sinaloa'
      WHEN cd.shipping_province = 'Chihuahua' THEN 'Chihuahua'
      WHEN cd.shipping_province = 'Tamaulipas' THEN 'Tamaulipas'
      WHEN cd.shipping_province = 'Michoacán' THEN 'Michoacán'
      WHEN cd.shipping_province = 'Quintana Roo' THEN 'Quintana Roo'
      WHEN cd.shipping_province = 'Coahuila' THEN 'Coahuila'
      WHEN cd.shipping_province = 'San Luis Potosí' THEN 'San Luis Potosí'
      ELSE 'Other'
    END
...
// =======================
// 📊 EXACT QUERY GENERATORS - RELATIVE TIME LOGIC FIXED
// =======================
...
// 4. SHARES ANALYSIS (CORE - OLD LOGIC PARA PERFORMANCE) 🔧 FIXED: Gateway/Shipping directo, solo Province multi-país
...
core_data AS (
  SELECT 
    ...
    -- 🏛️ SOLO Province Grouping usa lógica multi-país (como en la versión original)
    ${getProvinceGrouping()} AS shipping_province_grouping
    
  FROM filtered_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
  WHERE cd.special_date_name IN (ec.current_event, ec.last_event)
...
// 5. PROMOTIONS (CORE - RELATIVE TIME FIXED)
...
// 6A. TEMPORAL (CORE - RELATIVE TIME FIXED)
...
// 6B. LAST UPDATED (CORE - RELATIVE TIME FIXED)
...
// 7A. TRAFFIC (SESSIONS - RELATIVE TIME FIXED)
...
// 7B. SESSIONS LAST UPDATED (SESSIONS - RELATIVE TIME FIXED)
...
// 8A. PRODUCTS (PRODUCTS - RELATIVE TIME FIXED)
...
// 8B. PRODUCTS LAST UPDATED (PRODUCTS - RELATIVE TIME FIXED)
...
const items = queries.map(({ name, generator }) => ({
  json: {
    ...
    event_comparison: `${CURRENT_EVENT} vs ${LAST_EVENT}`,
    country: CFG.COUNTRY,
    event_days: CFG.EVENT_DAYS,
    events_table: CFG.EVENTS_TABLE[CFG.COUNTRY],
    tables_used: {
      actual: CFG.TABLES.ACTUAL[CFG.COUNTRY],
      historico: CFG.TABLES.HISTORICO[CFG.COUNTRY]
    },
    generated_at: new Date().toISOString(),
    temporal_fix: "MULTI_COUNTRY_READY_EVENT_DAYS_FILTER_FIXED_SHARES_GATEWAY_SHIPPING_DIRECT_PROVINCE_MULTICOUNTRY"
  }
}));
