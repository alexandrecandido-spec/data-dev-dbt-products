...
// Salida: items [{ needs_confirmation:false, sql:"<QUERY_MINIFICADA>", query_name:"<NOMBRE>" }, ...]
...
// =======================
// 📊 EXACT QUERY GENERATORS - FIXED TEMPORAL LOGIC
// =======================

// 1. GENERAL METRICS (CORE - FIXED)
...
core_data AS (
  SELECT 
    cd.special_date_name, cd.store_id,
...
core_data AS (
  SELECT 
    cd.special_date_name, cd.store_id, cd.business_unit,
    ec.current_event, ec.last_event
  FROM all_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
  WHERE cd.special_date_name IN (ec.current_event, ec.last_event)
    AND cd.business_unit IS NOT NULL
    AND (cd.special_date_day < cel.max_day OR 
         (cd.special_date_day = cel.max_day AND cd.special_date_hour < cel.max_hour))
)
...
core_data AS (
  SELECT 
    cd.store_id, cd.aging_years,
    ec.current_event
  FROM all_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
  WHERE cd.special_date_name = ec.current_event
    AND cd.aging_years IS NOT NULL
    AND (cd.special_date_day < cel.max_day OR 
         (cd.special_date_day = cel.max_day AND cd.special_date_hour < cel.max_hour))
)
...
core_data AS (
  SELECT 
    cd.special_date_name, cd.contact_email,
    ec.current_event, ec.last_event
  FROM all_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
...
core_data AS (
  SELECT 
    ...
    ec.current_event, ec.last_event,
    CASE
      WHEN cd.country = 'AR' AND UPPER(cd.shipping_province) IN (
      ...
      WHEN cd.country = 'AR' THEN 'resto del pais'
      ELSE NULL
...
core_data AS (
  SELECT 
    cd.special_date_name, cd.orders, cd.orders_with_any_promotion, cd.orders_free_shipping,
    cd.orders_promo_price, cd.orders_coupon,
    ec.current_event, ec.last_event
  FROM all_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
...
core_data AS (
  SELECT 
    cd.special_date_name, cd.orders, cd.products_quantity, cd.special_date_day, cd.special_date_hour,
    ec.current_event, ec.last_event
  FROM all_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
...
// 7A. TRAFFIC (SESSIONS - SEPARATE LIMIT)
...
all_sessions_data AS (
  SELECT * FROM ${CFG.TABLES.SESSIONS_ACTUAL}
  UNION ALL
  SELECT * FROM ${CFG.TABLES.SESSIONS_HISTORICO}
),
current_event_limit AS (
  SELECT 
    MAX(special_date_day) AS max_day,
    MAX(special_date_hour) AS max_hour
  FROM all_sessions_data
  CROSS JOIN events_config ec
  WHERE special_date_name = ec.current_event
),
core_sessions_data AS (
  SELECT 
    cd.special_date_name, cd.sessions, cd.orders, cd.carritos,
    ec.current_event, ec.last_event
  FROM all_sessions_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
  WHERE cd.special_date_name IN (ec.current_event, ec.last_event)
    AND (cd.special_date_day < cel.max_day OR 
...
// 7B. SESSIONS LAST UPDATED (SESSIONS - SEPARATE LIMIT)
...
FROM all_sessions_data ad
CROSS JOIN events_config ec
WHERE ad.special_date_name = ec.current_event`;

// 8A. PRODUCTS (PRODUCTS - SEPARATE LIMIT)
...
all_products_data AS (
  SELECT * FROM ${CFG.TABLES.PRODUCTS_ACTUAL}
  UNION ALL
  SELECT * FROM ${CFG.TABLES.PRODUCTS_HISTORICO}
),
current_event_limit AS (
  SELECT 
    MAX(special_date_day) AS max_day,
    MAX(special_date_hour) AS max_hour
  FROM all_products_data
  CROSS JOIN events_config ec
  WHERE special_date_name = ec.current_event
),
core_products_data AS (
  SELECT 
    cd.special_date_name, cd.product_type, cd.gmv, cd.products_quantity,
    ec.current_event, ec.last_event
  FROM all_products_data cd
  CROSS JOIN events_config ec
  CROSS JOIN current_event_limit cel
  WHERE cd.special_date_name IN (ec.current_event, ec.last_event)
...
// 8B. PRODUCTS LAST UPDATED (PRODUCTS - SEPARATE LIMIT)
...
const items = queries.map(({ name, generator }) => ({
  json: {
    ...
    temporal_fix: "fair_comparison_complete_hours_only"
  }
}));
