...
class DBTOperator(BashOperator):
    ...
    def execute(self, context):
        try:
            ...
            raise AirflowException(f"Error en la ejecución de DBT para el modelo {self.model}: {str(e)}")
    ...
    def _save_execution_state(self, context):
        ...
        state_file = f'/tmp/dbt/state/{self.model}_current_state.json'
    ...
    def _save_error_state(self, context, error_message):
        ...
        error_file = f'/tmp/dbt/state/{self.model}_error_state.json'
    ...
    def _update_success_state(self, context):
        ...
        success_file = f'/tmp/dbt/state/{self.model}_success_state.json'
