from airflow import DAG
from airflow.models import Variable
import ast
from datetime import datetime
from src.core.run_dbt_custom import create_dbt_dag
from src.core.utils.slack_manager import task_fail_slack_alert_bi
from functools import partial

SLACK_IDS= ast.literal_eval(Variable.get('slack_ids_analytics_engineer_alert'))



default_args = {
    'owner': 'Rodrigo Nicolás Torres',
    'depends_on_past': False,
    'start_date': datetime(2024, 10, 1),
    'email_on_failure': False,
    'email_on_retry': False,
    'on_failure_callback': partial(task_fail_slack_alert_bi,slack_ids=SLACK_IDS)

}


# Crear el DAG frecuencia diaria
dag = create_dbt_dag(
    dag_id='dbt_operations_daily-6am-only',
    schedule_interval_tag='daily-6am',
    default_args=default_args,
    tags=['operations','daily-6am']
)

# Crear el DAG frecuencia diaria
dag = create_dbt_dag(
    dag_id='dbt_operations_daily-8am',
    schedule_interval_tag='daily-8am',
    default_args=default_args,
    tags=['operations','daily-8am-8pm']
)

# Crear el DAG frecuencia diaria
dag = create_dbt_dag(
    dag_id='dbt_operations_daily-8pm',
    schedule_interval_tag='daily-8pm',
    default_args=default_args,
    tags=['operations','daily-8am-8pm']
)

# Crear el DAG frecuencia diaria
dag = create_dbt_dag(
    dag_id='dbt_operations_daily-9am-only',
    schedule_interval_tag='daily-9am',
    default_args=default_args,
    tags=['operations','daily-9am']
)

# Crear el DAG frecuencia diaria
dag = create_dbt_dag(
    dag_id='dbt_operations_daily-9am',
    schedule_interval_tag='daily-9am',
    default_args=default_args,
    tags=['operations','daily-9am-9pm']
)

# Crear el DAG frecuencia diaria
dag = create_dbt_dag(
    dag_id='dbt_operations_daily-9pm',
    schedule_interval_tag='daily-9pm',
    default_args=default_args,
    tags=['operations','daily-9am-9pm']
)

# Crear el DAG frecuencia mensual
dag = create_dbt_dag(
    dag_id='dbt_operations_monthly-1st-10AM',
    schedule_interval_tag='monthly-1st-10AM',
    default_args=default_args,
    tags=['operations','monthly-1st-10AM']
)

# Crear el DAG frecuencia diaria
dag = create_dbt_dag(
    dag_id='dbt_operations_daily-12pm',
    schedule_interval_tag='daily-12pm',
    default_args=default_args,
    tags=['operations','daily-12pm']
)

# Crear el DAG frecuencia semanal (mixta)
dag = create_dbt_dag(
    dag_id='dbt_operations_weekly-monday-9am',
    schedule_interval_tag='weekly-monday-9am',
    default_args=default_args,
    tags=['operations', 'weekly-monday-9am-monthly-1st-12pm']
)

# Crear el DAG frecuencia mensual (mixta)
dag = create_dbt_dag(
    dag_id='dbt_operations_monthly-1st-12pm',
    schedule_interval_tag='monthly-1st-12pm',
    default_args=default_args,
    tags=['operations', 'weekly-monday-9am-monthly-1st-12pm']
)

# Crear el DAG frecuencia semanal
dag = create_dbt_dag(
    dag_id='dbt_operations_weekly-monday-10am',
    schedule_interval_tag='weekly-monday-10am',
    default_args=default_args,
    tags=['operations', 'weekly-monday-10am']
)