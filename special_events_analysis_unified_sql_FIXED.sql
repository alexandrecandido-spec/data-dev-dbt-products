-- Databricks SQL — One row per order (AR), eventos locales (end_local inclusivo),
-- pagos "paid", blocked stores, SIN sessions, con max_date_order
-- Ventana: solo 2 días hacia atrás (-2 y -1) + MAIN (>= 0)
-- ✅ VERSIÓN CORREGIDA - Sin duplicados, formato partición correcto
...
-- =============================================================================
-- 📅 2. EVENTS & WINDOWS (✅ FECHA CORREGIDA)
-- =============================================================================
...
events_base AS (
  ...
  WHERE country IN ('AR')  -- HARDCODED: modificar según países
    AND start_date >= TIMESTAMP('2024-01-01T00:00:00')  -- ✅ CORREGIDO: 2024 no 2025
...
-- =============================================================================
-- 🛒 3. ORDERS SCOPED + PROMOCIONES (✅ PARTITION FORMAT CORREGIDO)
-- =============================================================================
...
-- ✅ ORDERS CON PROMOCIONES Y PARTITION PRUNING INTEGRADO
orders_scoped AS (
  SELECT 
    o.id AS order_pk,
    o.order_id,
    ...
    -- ✅ PROMOCIONES DIRECTAMENTE (sin query separada)
    ...
    -- Promociones simplificadas (sin cálculos pesados)
    0 AS descuento_pct,
  ...
  -- ✅ Payment date optimizada (PARTITION FORMAT CORREGIDO)
  LEFT JOIN (
    SELECT 
      order_id, 
    ...
    WHERE data_2 = 'paid'
      AND year_month_code IN (202510, 202511)  -- ✅ CORREGIDO: Sin comillas ni guión
    GROUP BY order_id
  ) pl ON pl.order_id = o.id
  
  -- Products quantity (sin partition porque no tiene year_month_code)
  LEFT JOIN (
    SELECT
      order_id,
  ...
  WHERE bs.store_id IS NULL
    ...
    -- ✅ PARTITION PRUNING CORREGIDO
    AND o.year_month_code IN (202510, 202511)  -- ✅ CORREGIDO: Sin comillas ni guión
    -- HARDCODED: order filters
    AND o.total_in_usd BETWEEN 0 AND 10000
    AND o.storefront NOT IN ('permalink')
),

-- ✅ EVENTO FILTERING OPTIMIZADO - INNER JOIN en lugar de EXISTS
orders_in_event_windows AS (
  SELECT 
    o.*,
    e.event_name,
    e.start_local,
  ...
  INNER JOIN event_windows_local e ON o.country_code = e.country  -- ✅ INNER JOIN optimizado
...
-- =============================================================================
-- 🎯 4. EVENT TAGGING & WINDOWS (PW vs MAIN) - SIN DUPLICADOS
-- =============================================================================
...
tagged_orders AS (
  ...
  FROM orders_in_event_windows o
),

-- ✅ RESULTADO CONSOLIDADO - Sin duplicados
final_tagged AS (
  SELECT
    t.order_pk,
    t.order_id,
    ...
    -- Métricas de promociones (YA INCLUIDAS)
    ...
    -- Fechas de evento originales para JOIN
    t.start_local,
    t.end_local,
    t.pw_start_day
...
-- =============================================================================
-- 🔗 5. UNIONES FINALES Y ENRIQUECIMIENTO - SIN DUPLICADOS
-- =============================================================================
...
-- =============================================================================
-- 🎯 RESULTADO FINAL ENRIQUECIDO (CON NUEVOS CAMPOS) - SIN DUPLICADOS
-- =============================================================================
...
enriched_final AS (
  SELECT
    ...
    -- ✅ NUEVO: Business Unit (MM vs SMB)
    CASE 
      WHEN m.group_name = 'enterprise' THEN 'MM'
      ELSE 'SMB'
    ...
    -- ✅ CORREGIDO: Gateway grouping solo para AR
    CASE
      WHEN m.country_code = 'AR' THEN
        CASE
    ...
    -- ✅ NUEVO: Shipping method grouping (solo AR)
    CASE
      WHEN m.country_code = 'AR' THEN
        CASE
          WHEN COALESCE(a2.handle, t.shipping_method) = 'custom' THEN 'Envío Personalizado'
          WHEN COALESCE(a2.handle, t.shipping_method) = 'correo-argentino' THEN 'Correo Argentino'
          WHEN COALESCE(a2.handle, t.shipping_method) = 'branch' THEN 'Retiro Sucursal'
          WHEN COALESCE(a2.handle, t.shipping_method) = 'andreani' THEN 'Andreani'
          WHEN COALESCE(a2.handle, t.shipping_method) IN ('oca','oca-partner-ar') THEN 'Oca'
          WHEN COALESCE(a2.handle, t.shipping_method) = 'envo-nube-argentina' THEN 'Envío Nube'
          ELSE 'Otros'
        END
      ELSE 'Other'
    END AS shipping_method_grouping,
...
-- =============================================================================
-- 🎯 SELECT FINAL - COLUMNAS OPTIMIZADAS PARA TABLEAU (CON NUEVOS CAMPOS)
-- =============================================================================
...
SELECT
  ...
  -- Business
  segment, business_size, plan_group, business_unit,  -- ✅ NUEVO CAMPO
  is_cartera_success, vertical_vertifier, vertical_grouping,
  ...
  gateway_installments, shipping_method_norm, shipping_method_grouping,  -- ✅ NUEVO CAMPO
  shipping_province,
...
FROM enriched_final
-- ❌ SIN ORDER BY para performance (hazlo en Tableau si es necesario)
