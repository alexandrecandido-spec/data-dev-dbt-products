...
FROM 
    ...
    left join {{ ref('_int_midmarket_wbr_mbr__weekly_status') }} st
      on d.store_id = st.store_id 
    left join {{ ref('_int_midmarket_wbr_mbr__weekly_franchise_group') }} fg
      on d.store_id = fg.store_id  
    left join {{ ref('_int_midmarket_wbr_mbr__weekly_main') }} m
