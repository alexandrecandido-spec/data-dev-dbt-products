...
-- Last MBR Stores (Historical Postgres)
{% if not is_incremental() %}
with postgres_last_month_stores as (
    select 
        store_id
    from 
        {{ ref('_int_midmarket_wbr_mbr__mbr_clean') }}
    where 
        date_from = (SELECT max(date_from) FROM {{ ref('_int_midmarket_wbr_mbr__mbr_clean') }})
...
{% if not is_incremental() %}
SELECT *
FROM {{ ref('_int_midmarket_wbr_mbr__mbr_clean') }}
...
-- Incremental 
SELECT 
    CAST(date_trunc('month', current_date - interval '1' month) AS DATE) AS date_from,
    CAST(date_trunc('month', current_date) - interval '1' day AS DATE) AS date_to,
    d.store_id,
...
FROM 
    ...
    left join {{ ref('_int_midmarket_wbr_mbr__free_days') }} fd
      ...
      and fd.periodicity = 'monthly'
WHERE 
    {% if is_incremental() %}
    (s.in_portfolio = true
        OR s.store_id in (select store_id from last_month_stores))
    and CAST(date_trunc('month', current_date - interval '1' month) AS DATE) > (
    ...
    {% if not is_incremental() %}
    (s.in_portfolio = true
        OR s.store_id in (select store_id from postgres_last_month_stores))
    and CAST(date_trunc('month', current_date - interval '1' month) AS DATE) > (
        SELECT max(date_from) 
        FROM {{ ref('_int_midmarket_wbr_mbr__mbr_clean') }}
        )
