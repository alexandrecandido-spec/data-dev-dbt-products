...
(
  ...
  FROM  comments tc
  LEFT JOIN {{ source('dp_zendesk_support_prod', 'users') }} ud ON ud.id = tc.author_id
...
(
  ...
  FROM
  (
   SELECT ticket_id, event_id ,event_timestamp AS effective_from, cast(field_value as bigint) as assignee_id
...
(
  ...
  FROM
  (
   SELECT ticket_id, event_id ,event_timestamp AS effective_from, cast(field_value as bigint) as group_id
   FROM   {{ ref('s__general__zendesk_tickets_events__event') }}
...
FROM prev_io a
LEFT JOIN assignee_id b on a.ticket_id = b.ticket_id and a.interaction_timestamp between b.effective_from and b.effective_to
LEFT JOIN group_id    c on a.ticket_id = c.ticket_id and a.interaction_timestamp between c.effective_from and c.effective_to
-- WHERE a.ticket_id = 6394275
-- ORDER BY a.ticket_id, a.ticket_interaction_rank
)
