...
LEFT JOIN {{ ref('dim_location_country') }} AS b ON b.country_code = a.country
LEFT JOIN {{ ref('_int_company_metrics_merchant_info__location_by_zipcode') }} AS c ON a.store_id = c.store_id
LEFT JOIN {{ ref('_int_company_metrics_merchant_info__segment') }} AS d ON a.store_id = d.store_id
LEFT JOIN {{ ref('_int_company_metrics_merchant_info__vertifier_and_vertical') }} e ON a.store_id = e.store_id
LEFT JOIN {{ ref('_int_company_metrics_merchant_info__business_size') }} f ON a.store_id = f.store_id
LEFT JOIN {{ ref('dim_group_plan') }} g ON array_contains(g.plan_id_nk, a.plan)
)
