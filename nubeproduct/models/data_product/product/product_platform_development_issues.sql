...
issues_final as (
    select 
        ...
        */
        ...
        ,sum(avg_gmv_lc_last_3months) as impacted_gmv_local
...
select 
    concat(cast(p.ticket_number as string), p.labels_country) as unique_issue_country
    ,p.* 
    ,{% if is_incremental() %}
...
left join {{ this }} existing
    ...
    and p.labels_country = existing.labels_country
{% endif %}