...
installs AS (
    ...
    {% if is_incremental() %}
      ...
      WHERE sys_audit_updated_at >= (SELECT COALESCE(MAX(sys_audit_updated_at), '1900-01-01') FROM {{ this }})
    {% endif %}
),
install_lag AS (
    SELECT 
...
LEFT JOIN {{ this }} target
    ON concat(cast(store_id as string), cast(app_id as string)) = target.store_app_id
{% endif %}
GROUP BY 1,2,3