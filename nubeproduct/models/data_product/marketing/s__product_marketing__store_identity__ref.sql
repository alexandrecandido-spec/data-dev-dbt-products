...
Este modelo consolida información de identidad de la tienda desde múltiples fuentes:
...
main_user AS (
    SELECT
        ...
        wu.user_role,
        ROW_NUMBER() OVER (PARTITION BY wu.store_id ORDER BY wu.id DESC) AS rn
...
LEFT JOIN main_user mu ON sb.store_id = mu.store_id AND sb.main_user_id = mu.user_id AND mu.rn = 1
LEFT JOIN theme_latest tl ON sb.store_id = tl.store_id AND tl.rn = 1
LEFT JOIN existing_data ed ON sb.store_id = ed.store_id
{% if is_incremental() %}
