...
WITH source_data AS (
select related_id as store_id, tag from (
 select related_id, tag,
 	row_number() over (partition by related_id order by created desc) rownum
		from {{ source('stg_moltres', 'mwp_tags') }} mt 
