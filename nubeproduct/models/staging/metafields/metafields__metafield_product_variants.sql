...
with metafield_product_variants as (
    SELECT 
    uuid,
    namespace,
    key,
    ...
    read_only,
    app_id
    FROM {{ source('stg_metafields', 'metafield_product_variants') }} AS mpv
    {% if is_incremental() %}
...
SELECT 
    metafield_product_variants.uuid,
    namespace,
    key,
    store_id,
    owner_resource,
...
LEFT JOIN existing_data e ON metafield_product_variants.uuid = e.uuid