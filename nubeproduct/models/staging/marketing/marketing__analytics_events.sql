...
SELECT 
    ...
    year_month_code
FROM 
    {{ source('stg_marketing', 'analytics_events') }} AS events
{% if is_incremental() %}
WHERE
   event_date >= (select max(event_date)- 5 from {{ this }} )
...
existing_data AS (
    {{ get_existing_data(this, ['event_bundle_sequence_id', 'event_timestamp', 'user_pseudo_id', 'batch_event_index', 'event_name', 'sys_audit_created_on', 'sys_audit_created_by']) }}
)
...
SELECT 
    ...
    CONCAT(d.source, '-', d.user_pseudo_id, '-', CAST(
        COALESCE(
