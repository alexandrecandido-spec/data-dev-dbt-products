...
apps.has_nuvempago,
apps.has_pagonube,
apps.total_orders_last_365d,
apps.total_np_orders_last_365d,
apps.total_pn_orders_last_365d,
apps.total_np_orders_since_rollout,
apps.total_pn_orders_since_rollout,
...
count(distinct cmpo.id) as total_orders_last_365d,
count(distinct case when payment='nuvem-pago' then cmpo.id end) as total_np_orders_last_365d,
count(distinct case when payment='pago-nube' then cmpo.id end) as total_pn_orders_last_365d,
count(distinct case when payment='nuvem-pago' and TO_DATE(cast(year_month_day_code as string), 'yyyyMMdd')>=date('2025-09-15')  then cmpo.id end) as total_np_orders_since_rollout,
count(distinct case when payment='pago-nube' AND TO_DATE(cast(year_month_day_code as string), 'yyyyMMdd')>=date('2025-09-15')  then cmpo.id end) as total_pn_orders_since_rollout,
