with
	sales_sqls as
		(select
            ...
            coalesce(s.sales_date_entered_scheduled_meetings_week, sales_date_entered_problem_discovery_week) as date_week,
		...
		where 
		    pipeline in 
                ...
                'Sales | BR',
                'Sales | MX')
            and coalesce(s.sales_date_entered_scheduled_meetings_week, s.sales_date_entered_problem_discovery_week) is not null
	...
	sales_opportunities as
		(select
            ...
            s.sales_date_entered_problem_discovery_week as date_week,
            --case when s.country = 'AR' then s.amount / fc.value_local_currency else null end as potential_gmv_usd,
            /*case
                when s.country = 'BR' then
                    case 
		...
		where 
		    pipeline in 
                ...
                'Sales | BR',
                'Sales | MX')
            and s.sales_date_entered_problem_discovery_week is not null
