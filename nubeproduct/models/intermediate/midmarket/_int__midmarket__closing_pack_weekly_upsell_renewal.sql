with 
	...
	from {{ ref('midmarket__general__closing_pack_hubspot_deals') }} as r
		    left join {{ ref('midmarket_weekly_business_review') }} as wbr
				on r.store_id = wbr.store_id 
				and r.close_week = wbr.date_from
		    ...
		    left join {{ ref('company_metrics_weekly_snapshot_gmv') }} as gmv
		        ...
		        and r.close_week = gmv.date_from
			where 
				r.pipeline in 
					('Upsell Success | AR',
					'Upsell Success | BR',
	...
	upsell as
		...
		from {{ ref('midmarket__general__closing_pack_hubspot_deals') }} as u
		    ...
		    left join {{ ref('company_metrics_weekly_snapshot_gmv') }} as gmv
		        ...
		        and u.close_week = gmv.date_from
			where 
				u.pipeline in 
					('Upsell Success | AR',
					'Upsell Success | BR',
