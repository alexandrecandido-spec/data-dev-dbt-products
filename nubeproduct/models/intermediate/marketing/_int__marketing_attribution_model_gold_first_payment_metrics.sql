SELECT
    ss.first_payment as date, --- consolidated_date a charlar con Ron
    att.store_id,
    att.country_code,
    att.source,
    att.medium,
...
-- sellers no aplican en esta CTE
...
-- new payments por first_payment
    sum(case when ss.new_payment = true then trials_last_click  else 0 end) as new_payment_last_click,
    sum(case when ss.new_payment = true then trials_first_click else 0 end) as new_payment_first_click,
    sum(case when ss.new_payment = true then trials_mean_click  else 0 end) as new_payment_mean_click,



    sum(case when ss.new_seller = true then trials_last_click  else 0 end) as new_seller_fpd_last_click,
    sum(case when ss.new_seller = true then trials_first_click else 0 end) as new_seller_fpd_first_click,
    sum(case when ss.new_seller = true then trials_mean_click  else 0 end) as new_seller_fpd_mean_click,
...
WHERE ss.first_payment is not null
AND ss.first_payment >= '2018-01-01'
