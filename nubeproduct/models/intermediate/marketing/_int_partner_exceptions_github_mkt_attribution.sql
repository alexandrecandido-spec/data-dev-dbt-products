...
utm_issues AS (
    ...
    WHERE mkt_git.input_title like '%PARTNER_CODE%'
)
...
SELECT
    ...
    input_number,
    input_title,
    CASE WHEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*partner_code\\*\\*:'), 2),'\\*\\*team\\*\\*:'), 1)), '\\s*-\\s*$', '')) = 'nan' THEN NULL
        ELSE TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*partner_code\\*\\*:'), 2),'\\*\\*team\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS partner_code,
