...
SELECT
    ...
    input_title,
    issue_type,
    CASE WHEN issue_type IN ('UTM', 'SUBTEAM_MKT') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*utm_source\\*\\*:'), 2),'\\*\\*utm_medium\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS utm_source,
    CASE WHEN issue_type IN ('UTM', 'SUBTEAM_MKT') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*utm_medium\\*\\*:'), 2),'\\*\\*utm_campaign\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS utm_medium,
    CASE WHEN issue_type IN ('UTM', 'SUBTEAM_MKT') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*utm_campaign\\*\\*:'), 2),'\\*\\*utm_content\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS utm_campaign,
    CASE WHEN issue_type IN ('UTM', 'SUBTEAM_MKT') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*utm_content\\*\\*:'), 2),'\\*\\*team\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS utm_content,
    CASE WHEN issue_type IN ('URL') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*url\\*\\*:'), 2),'\\*\\*landing_page_domain\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS url,
    CASE WHEN issue_type IN ('URL') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*landing_page_domain\\*\\*:'), 2),'\\*\\*landing_page_path\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS landing_page_domain,
    CASE WHEN issue_type IN ('URL') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*landing_page_path\\*\\*:'), 2),'\\*\\*team\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS landing_page_path,
    CASE WHEN issue_type IN ('REFERRER') THEN TRIM(REGEXP_REPLACE(TRIM(element_at(split(element_at(split(body, '\\*\\*referrer\\*\\*:'), 2),'\\*\\*team\\*\\*:'), 1)), '\\s*-\\s*$', '')) END AS referrer,
