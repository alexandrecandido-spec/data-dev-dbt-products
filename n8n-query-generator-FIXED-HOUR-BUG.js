...
// CORREGIDO: Lógica de tiempo relativo para comparaciones justas entre eventos
...
// Helper function to generate relative time logic for any data source
...
// 4. SHARES ANALYSIS (CORE - RELATIVE TIME FIXED)
const generateShares = () => `
WITH ${getRelativeTimeLogic("CORE")},
core_data AS (
  SELECT 
    wr.special_date_name, wr.vertical_grouping, wr.gateway_provider_grouping, wr.shipping_method_grouping,
    wr.shipping_province, wr.country, wr.gateway_installments, wr.Order_Source, wr.Social_Network,
    wr.business_unit, wr.team_last_click, wr.gmv, wr.orders,
    ec.current_event, ec.last_event,
    CASE
      WHEN wr.country = 'AR' AND UPPER(wr.shipping_province) IN (
      ...
      WHEN wr.country = 'AR' THEN 'resto del pais'
      ELSE NULL
